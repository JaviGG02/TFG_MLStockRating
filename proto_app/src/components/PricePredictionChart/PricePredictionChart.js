...
const PricePredictionChart = ({ datosFinancieros, predicciones }) => {
  ...
  const fechasReales = datosFinancieros
    ? Object.keys(datosFinancieros.TIME_SERIES_MONTHLY_ADJUSTED)
    : [];
  ...
  const preciosRealesRellenados = fechasUnificadas.map((fecha) =>
    fechasReales.includes(fecha)
      ? datosFinancieros.TIME_SERIES_MONTHLY_ADJUSTED[fecha]
      : null,
  );
  const preciosPrediccionesRellenados = fechasUnificadas.map((fecha) =>
    fechasPredicciones.includes(fecha) ? predicciones[fecha] : null,
  );
