...
def map_fundamentals(ticker, df_prices, remove_prefix="us-gaap_"):
    ...
    try:
        datos_finnhub = finnhub_client.financials_reported(
    ...
    for quarterly_fundamentals in datos_finnhub:
        ...
        for fundamental in quarterly_fundamentals["report"]:
            for topic in quarterly_fundamentals["report"][fundamental]:
                ...
                if (
                    datos_calculos["unit"] == None
                ):  # Guardamos la unidad de la moneda empleada
        ...
        operaciones = [
            ...
            (
                "investments",
                "suma",
                ("MarketableSecuritiesCurrent", "EquityMethodInvestments"),
            ),
            (
                "currentNetReceivables",
                "suma",
                ("ShareBasedCompensation", "AccountsReceivableNetCurrent"),
            ),
        ...
        for (
            ...
            if destino not in datos_mapeados:
                datos_mapeados[destino] = calcular_fundamentales(
                    operacion, claves, datos_calculos
                )
        ...
        datos_mapeados["1y_sharePrice"] = closest_price_from_df(
            date_1y, df_prices, ticker
        )
...
for i, ticker in enumerate(tickers):
    ...
    try:
        adjusted_close = {
            ...
            for date, values in prices["Monthly Adjusted Time Series"].items()
        }
    except KeyError:
        continue
    diccionario_precios = {}
    ...
    prices_df = prices_df.merge(
        prices_symbol, left_index=True, right_index=True, how="outer"
    )
